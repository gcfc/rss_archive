...
def test_all_algorithms(csv_file_path, template_file_path, output_file_path, swap=False):
    ...
    #"template_matching": cd_template_matching,
                        #"SIFT": cd_sift_ransac})
    total_data = []
    ...
    if len(sys.argv) == 1:
        ...
        if scores:
            ...
            for (img, val) in scores.iteritems():
                ...
                temp += val
                count += 1
            print('Average: ',temp/count)



        scores = {int(key.split("/")[2][4:-4]):scores[key] for key in scores.keys()}
        keys = scores.keys()
        keys.sort()
        scores = {key:scores[key] for key in keys}

        plt.bar(range(1, len(scores.values()) + 1), scores.values())
        plt.xticks(range(1, len(scores.values()) + 1), scores.keys(), rotation = 90)
        plt.xlabel('Image Number')
        plt.ylabel('Intersection over Union')
        plt.ylim(0.5, 1.0)
        plt.title('IOU Values for Color Bounds: [5, 50, 50], [15, 255, 255] \n Average: {}'.format(temp/count))
        # plt.plot(range(1,len(scores.values())+1), scores.values(), 'ko-')
        plt.show()
